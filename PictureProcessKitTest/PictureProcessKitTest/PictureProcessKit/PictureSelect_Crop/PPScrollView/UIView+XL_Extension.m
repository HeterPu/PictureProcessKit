...
@implementation UIView (XL_Extension)
 ...
 *  @param redY y坐标
 */
- (void)xl__showRedTipViewInRedX:(CGFloat)redX redY:(CGFloat)redY
{
    [self xl__showRedTipViewInRedX:redX redY:redY redTipViewWidth:5];
}
...
{
    [self xl__showRedTipViewInRedX:redX redY:redY redTipViewWidth:width backgroundColor:[UIColor redColor]];
}
...
- (void)xl__showRedTipViewInRedX:(CGFloat)redX redY:(CGFloat)redY redTipViewWidth:(CGFloat)width backgroundColor:(UIColor *)backgroundColor
{
    if (!self.redTipView) {
        ...
        self.redTipView.xl__width = width;
        self.redTipView.xl__height = width;
        self.redTipView.layer.cornerRadius = self.redTipView.xl__height * 0.5;
    ...
    self.redTipView.xl__x = redX;
    self.redTipView.xl__y = redY;
...
- (void)setXl__size:(CGSize)xl_size
{
...
- (void)setXl__width:(CGFloat)xl_width
{
...
- (void)setXl__height:(CGFloat)xl_height
{
    CGRect rect = self.frame;
...
- (void)setXl__centerX:(CGFloat)xl_centerX
{
...
- (void)setXl__centerY:(CGFloat)xl_centerY
{
    CGPoint point = self.center;
    point.y = xl_centerY;
    self.center = point;
...
{
    // 主窗口
    UIWindow *keyWindow = XL_KeyWindow;
...
{
    NSNumber *animationScale1 = type == XL_AnimationTypeToBigger ? @(1.15) : @(0.5);
    NSNumber *animationScale2 = type == XL_AnimationTypeToBigger ? @(0.92) : @(1.15);
